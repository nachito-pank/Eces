...
export default function EnseignantDetailPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [enRes, filRes] = await Promise.all([
        ...
        const data = {
          ...
          matieres: Array.isArray(enRes.enseignant.matieres)
            ...
            : [enRes.enseignant.matieres].filter(Boolean),
        };
        setEnseignant(data);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...formData,
        matieres: formData.matieresStr.split(',').map((m: string) => m.trim()).filter(Boolean),
      };
      delete payload.matieresStr;
      const res = await enseignantsApi.update(id, payload);
      const updated = { ...res.enseignant, matieres: Array.isArray(res.enseignant.matieres) ? res.enseignant.matieres : [res.enseignant.matieres].filter(Boolean) };
  ...
  const toggleFiliere = (nom: string) => {
    ...
    setFormData((prev: any) => ({
      ...prev,
      filieres: prev.filieres.includes(nom)
        ? prev.filieres.filter((f: string) => f !== nom)
        : [...prev.filieres, nom],
    }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-1 lg:sticky lg:top-8">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="px-6 pb-6 relative">
              ...
              <div className="pt-14">
                <h2 className="text-xl font-bold text-slate-900">{enseignant.prenom} {enseignant.nom}</h2>
                ...
                <div className="mt-5 space-y-2.5">
                  <div className="flex items-center text-sm text-slate-600"><Mail className="w-4 h-4 mr-3 text-slate-400 shrink-0" /><span className="truncate">{enseignant.email}</span></div>
                  <div className="flex items-center text-sm text-slate-600"><Phone className="w-4 h-4 mr-3 text-slate-400 shrink-0" />{enseignant.telephone || 'Non renseigné'}</div>
                  <div className="flex items-center text-sm text-slate-600"><Calendar className="w-4 h-4 mr-3 text-slate-400 shrink-0" />Embauché le {enseignant.dateEmbauche}</div>
                </div>
        ...
        <div className="lg:col-span-2 space-y-5">
              ...
              {[
                ...
                <div key={name}>
                  ...
                  {isEditing ? (
                    ...
                    <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-900">{enseignant[name] || '—'}</div>
                  )}
                </div>
              ))}
              <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 sm:p-6">
            <div className="flex items-center mb-5">
              <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center mr-3"><BookOpen className="w-4 h-4 text-blue-600" /></div>
              <h3 className="text-lg font-semibold text-slate-900">Informations académiques</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                  Matières <span className="text-xs text-slate-400 font-normal">(séparées par des virgules)</span>
                </label>
                {isEditing ? (
                  <Input type="text" name="matieresStr" value={formData.matieresStr} onChange={handleChange}
                    className="block w-full px-3 py-2 border border-blue-300 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm" />
                ) : (
                  <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-900">{enseignant.matieres?.join(', ')}</div>
                )}
              </div>
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Filières</label>
                {isEditing ? (
                  <div className="grid grid-cols-2 gap-2 p-3 border border-blue-300 rounded-xl bg-white">
                    {filieres.map((f: any) => (
                      <label key={f.id} className="flex items-center cursor-pointer gap-2 py-0.5">
                        ...
                        <span className="text-sm text-slate-700 truncate">{f.nom}</span>
                      </label>
                    ))}
                  </div>
                ) : (
                  <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-900">{enseignant.filieres?.join(', ')}</div>
                )}
              </div>
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-900">{enseignant.statut}</div>
                )}
              </div>
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-900">{enseignant.dateEmbauche}</div>
                )}
              </div>
            </div>
          </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <h3 className="text-lg font-bold text-center text-slate-900 mb-2">Supprimer l&apos;enseignant</h3>
            <p className="text-sm text-center text-slate-500 mb-6">
              Êtes-vous sûr de vouloir supprimer <strong>{enseignant.prenom} {enseignant.nom}</strong> ?
            </p>
