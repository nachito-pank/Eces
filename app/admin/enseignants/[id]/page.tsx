...
import { ChevronRight, Mail, Phone, Calendar, User, BookOpen, Edit2, Trash2, Check, X, AlertTriangle, Loader2, AlertCircle } from 'lucide-react';
...
export default function EnseignantDetailPage() {
  ...
  useEffect(() => {
    Promise.all([enseignantsApi.getById(id), filieresApi.getAll()])
      .then(([enRes, filRes]) => {
        const data = { ...enRes.enseignant, matieres: Array.isArray(enRes.enseignant.matieres) ? enRes.enseignant.matieres : [] };
        setEnseignant(data);
        setFormData({ ...data, matieresStr: data.matieres.join(', ') });
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const payload = { ...formData, matieres: formData.matieresStr.split(',').map((m: string) => m.trim()).filter(Boolean) };
      delete payload.matieresStr;
      const res = await enseignantsApi.update(id, payload);
      const updated = { ...res.enseignant, matieres: Array.isArray(res.enseignant.matieres) ? res.enseignant.matieres : [] };
      setEnseignant(updated);
      setFormData({ ...updated, matieresStr: updated.matieres.join(', ') });
  ...
  const handleDelete = async () => {
    try { await enseignantsApi.delete(id); router.push('/admin/enseignants'); }
  ...
  const toggleFiliere = (nom: string) => {
    if (!isEditing) return;
    setFormData((p: any) => ({ ...p, filieres: p.filieres.includes(nom) ? p.filieres.filter((f: string) => f !== nom) : [...p.filieres, nom] }));
  };
  ...
  if (error && !enseignant) return <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-xl flex items-center"><AlertCircle className="w-5 h-5 mr-3" />{error}</div>;
  if (!enseignant) return null;

  return (
    <div className="space-y-6">
      <nav className="flex text-sm text-slate-500 dark:text-slate-400 font-medium flex-wrap gap-1">
        <Link href="/admin/enseignants" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Enseignants</Link>
        <ChevronRight className="w-4 h-4 mx-1 shrink-0" />
        <span className="text-slate-900 dark:text-slate-100">{enseignant.prenom} {enseignant.nom}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-1 lg:sticky lg:top-8">
          <div className={`${PANEL} overflow-hidden`}>
            ...
            <div className="px-6 pb-6 relative">
              ...
              <div className="pt-14">
                <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">{enseignant.prenom} {enseignant.nom}</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400">{enseignant.matieres?.join(', ')}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${enseignant.statut === 'Actif' ? 'bg-green-50 dark:bg-green-900/40 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/40 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800'}`}>{enseignant.statut}</span>
                </div>
                <div className="mt-5 space-y-2.5">
                  {[{ Icon: Mail, val: enseignant.email }, { Icon: Phone, val: enseignant.telephone || 'Non renseigné' }, { Icon: Calendar, val: `Embauché le ${enseignant.dateEmbauche}` }].map(({ Icon, val }) => (
                ...
                <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2">
                  {isEditing ? (
                    <>
                      <button onClick={handleSave} disabled={saving} className="w-full flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-60">
                        {saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Check className="w-4 h-4 mr-2" />} Enregistrer
                      </button>
                      <button onClick={() => { setIsEditing(false); setFormData({ ...enseignant, matieresStr: enseignant.matieres.join(', ') }); }}
                    ...
                    <>
                      <button onClick={() => setIsEditing(true)} className="w-full flex items-center justify-center px-4 py-2 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-sm font-medium rounded-xl transition-colors">
                        <Edit2 className="w-4 h-4 mr-2" /> Modifier le profil
                      </button>
                      <button onClick={() => setShowDeleteModal(true)} className="w-full flex items-center justify-center px-4 py-2 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 text-sm font-medium rounded-xl transition-colors">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className={`${PANEL} p-5 sm:p-6`}>
            <div className="flex items-center mb-5">
              <div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/40 flex items-center justify-center mr-3"><BookOpen className="w-4 h-4 text-blue-600 dark:text-blue-400" /></div>
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Informations académiques</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                {isEditing ? <Input type="text" name="matieresStr" value={formData.matieresStr} onChange={handleChange} className={FEDIT} />
                  : <div className={FVIEW}>{enseignant.matieres?.join(', ')}</div>}
              </div>
              <div className="sm:col-span-2">
                ...
                {isEditing ? (
                  <div className="grid grid-cols-2 gap-2 p-3 border border-blue-300 dark:border-blue-600 rounded-xl bg-white dark:bg-slate-700">
                    {filieres.map((f: any) => (
                      <label key={f.id} className="flex items-center cursor-pointer gap-2 py-0.5">
                        <input type="checkbox" checked={formData.filieres?.includes(f.nom)} onChange={() => toggleFiliere(f.nom)}
                ...
                ) : <div className={FVIEW}>{enseignant.filieres?.join(', ')}</div>}
              </div>
              <div>
              ...
              <div>
                ...
                {isEditing ? <Input type="date" name="dateEmbauche" value={formData.dateEmbauche} onChange={handleChange} className={FEDIT} />
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`${PANEL} max-w-md w-full p-6`}>
            ...
            <h3 className="text-lg font-bold text-center text-slate-900 dark:text-slate-100 mb-2">Supprimer l&apos;enseignant</h3>
            <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">Êtes-vous sûr de vouloir supprimer <strong>{enseignant.prenom} {enseignant.nom}</strong> ?</p>
