...
export default function FilieresPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setFilieres((p) => p.map((f) => (f.id === editingId ? res.filiere : f)));
      setEditingId(null);
    } catch (err: any) {
      setError(err.message);
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setFilieres((p) => p.filter((f) => f.id !== showDeleteModal));
  ...
  const inputCls =
    "block w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 text-sm";
  const editInputCls =
    "block w-full px-3 py-2 border border-blue-300 dark:border-blue-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 text-sm";
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Gérez les filières d&apos;enseignement
          </p>
        </div>
        <button
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-xl flex items-center">
          ...
          <button onClick={fetchFilieres} className="ml-auto text-xs underline">
            Réessayer
          </button>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {filieres.map((filiere, index) => {
            ...
            return (
              <div
                key={filiere.id}
                className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden relative group"
              >
                ...
                {isEditing ? (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEditData({ ...editData, cycle: e.target.value })
                          }
                          className={editInputCls}
                        >
                          <option value="">--</option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEditData({ ...editData, niveau: e.target.value })
                          }
                          className={editInputCls}
                        >
                          <option value="">--</option>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setEditData({
                              ...editData,
                              nbEtudiants: Number(e.target.value),
                            })
                          }
                          className={editInputCls}
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setEditData({
                              ...editData,
                              nbEnseignants: Number(e.target.value),
                            })
                          }
                          className={editInputCls}
                        />
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 mb-2">
                          {filiere.code}
                        </span>
                        <h3 className="text-lg font-title font-bold text-slate-900 dark:text-slate-100 leading-tight">
                          {filiere.nom}
                        </h3>
                      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto border dark:border-slate-700">
            <h3 className="text-lg font-title font-bold text-slate-900 dark:text-slate-100 mb-4">
              Ajouter une filière
            </h3>
            <form onSubmit={handleAddSubmit} className="space-y-4">
              {[
                { label: "Nom *", key: "nom", ph: "Ex: Sciences Politiques" },
                { label: "Code *", key: "code", ph: "Ex: SCP" },
              ].map(({ label, key, ph }) => (
                <div key={key}>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: "Cycle *", key: "cycle", opts: CYCLES },
                  { label: "Niveau *", key: "niveau", opts: NIVEAUX },
                ].map(({ label, key, opts }) => (
                  <div key={key}>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                      {label}
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: "Nb Étudiants", key: "nbEtudiants" },
                  { label: "Nb Enseignants", key: "nbEnseignants" },
                ].map(({ label, key }) => (
                  <div key={key}>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                      {label}
              ...
              <div className="flex gap-3 mt-2">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-xl transition-colors"
                >
                  Annuler
                </button>
                <button
