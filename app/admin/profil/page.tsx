...
export default function ModificationIdentifiantsPage() {
  ...
  useEffect(() => {
    profilApi.get()
      .then((res) => {
        const a = res.admin;
        const d = { nom: a.nom, email: a.email, motDePasse: '' };
        setAdminData(d);
        setOriginalData(d);
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const passwordRequested = passwords.ancien || passwords.nouveau || passwords.confirmation;
    ...
    try {
      ...
      if (passwordRequested) payload.motDePasse = passwords.nouveau;

      await profilApi.update(payload);

      const updated = { ...adminData, motDePasse: passwordRequested ? passwords.nouveau : adminData.motDePasse };
  ...
  return (
    <div className="space-y-6 px-0 sm:px-0">
      <div>
        <h1 className="text-3xl font-title font-bold text-slate-900">Identifiants</h1>
        <p className="text-slate-500 mt-1">Gérez vos informations de connexion administrateur</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-1 lg:sticky lg:top-8">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 text-center">
            ...
            <h2 className="text-xl font-title font-bold text-slate-900">{adminData.nom}</h2>
            <p className="text-sm text-slate-500 mb-5">{adminData.email}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 sm:p-6">
              <div className="flex items-center mb-5">
                ...
                <h3 className="text-lg font-title font-semibold text-slate-900">Informations du compte</h3>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input type="text" name="nom" required value={adminData.nom} onChange={handleAdminChange}
                      className="block w-full pl-10 pr-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Email de connexion</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 sm:p-6">
              <div className="flex items-center mb-5">
                ...
                <h3 className="text-lg font-title font-semibold text-slate-900">Modifier le mot de passe</h3>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Ancien mot de passe</label>
