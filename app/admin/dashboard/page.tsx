...
import {
  ...
  Plus, ArrowRight, Settings, Loader2
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const statCards = [
    { title: 'Enseignants', value: stats.enseignants, icon: Users, bg: 'bg-blue-500', iconBg: 'bg-blue-100', iconColor: 'text-blue-500', href: '/admin/enseignants' },
    { title: 'Sous-Admins', value: stats.sousAdmins, icon: UserCog, bg: 'bg-emerald-500', iconBg: 'bg-emerald-100', iconColor: 'text-emerald-500', href: '/admin/sous-admins' },
    { title: 'Filières', value: stats.filieres, icon: BookOpen, bg: 'bg-amber-500', iconBg: 'bg-amber-100', iconColor: 'text-amber-500', href: '/admin/filieres' },
    { title: 'Étudiants', value: stats.etudiants, icon: GraduationCap, bg: 'bg-purple-500', iconBg: 'bg-purple-100', iconColor: 'text-purple-500', href: '/admin/etudiants' },
  ];
    ...
    <div className="space-y-8 p-3 sm:p-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="space-y-3">
              {recentEnseignants.map((enseignant) => (
                <div key={enseignant.id}
                  className="flex items-center p-3 sm:p-4 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 cursor-pointer"
                  ...
                  <div className="ml-3 flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-900 truncate">{enseignant.prenom} {enseignant.nom}</p>
                    <p className="text-xs text-slate-500 truncate">{enseignant.matieres?.join(', ')}</p>
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
          <h2 className="text-lg font-title font-semibold text-slate-900 mb-5">Raccourcis</h2>
          <div className="space-y-3">
            {[
