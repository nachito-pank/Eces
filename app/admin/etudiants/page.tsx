      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Gérez les inscriptions et le suivi des étudiants
          </p>
        </div>
        <Link
              ...
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50">
                  {[
                    ...
                    <th
                      key={h}
                      className={`text-left p-4 font-semibold text-slate-900 dark:text-slate-200 whitespace-nowrap ${i === 2 ? "hidden md:table-cell" : ""} ${i === 3 ? "hidden lg:table-cell" : ""} ${i === 5 ? "text-right" : ""}`}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {filtered.map((etudiant) => (
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            backgroundColor: etudiant.couleur || "#3B82F6",
                          }}
                        >
                          {etudiant.avatar}
                        </div>
                    ...
                    <td className="p-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statutClass(etudiant.statut)}`}
                      >
                        {etudiant.statut}
                      </span>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          onClick={() =>
                            router.push(`/admin/etudiants/${etudiant.id}`)
                          }
