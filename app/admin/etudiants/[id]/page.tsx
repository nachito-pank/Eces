...
const FVIEW =
  "px-3 py-2 bg-slate-50 dark:bg-slate-700/60 rounded-xl text-sm text-slate-900 dark:text-slate-100";
const FEDIT =
  "block w-full px-3 py-2 border border-blue-300 dark:border-blue-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 text-sm";
const LABEL =
        ...
        <div className="lg:col-span-1 lg:sticky lg:top-8">
          <div className={`${PANEL} overflow-hidden`}>
            ...
            <div className="px-6 pb-6 relative">
              <div
                className="w-20 h-20 rounded-2xl flex items-center justify-center text-white font-bold text-2xl border-4 border-white dark:border-slate-800 shadow-sm absolute -top-10"
              ...
              <div className="pt-14">
                ...
                <code className="mt-2 inline-block bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-xs font-mono text-slate-700 dark:text-slate-300">
                  {etudiant.matricule}
                </code>
                <div className="mt-3">
                  <span
                    className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${etudiant.statut === "Actif" ? "bg-green-50 dark:bg-green-900/40 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800" : "bg-red-50 dark:bg-red-900/40 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800"}`}
                  >
                    {etudiant.statut}
                  </span>
                </div>
                <div className="mt-5 space-y-2.5">
                  <div className="flex items-center text-sm text-slate-600 dark:text-slate-300">
                    <Mail className="w-4 h-4 mr-3 text-slate-400 dark:text-slate-500 shrink-0" />
                    <span className="truncate">{etudiant.email}</span>
                  </div>
                  <div className="flex items-center text-sm text-slate-600 dark:text-slate-300">
                    <Phone className="w-4 h-4 mr-3 text-slate-400 dark:text-slate-500 shrink-0" />
                  ...
                  {isEditing ? (
                    <>
                      <button
                        onClick={handleSave}
                        disabled={saving}
                        className="w-full flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl disabled:opacity-60"
                      >
                        {saving ? (
                    ...
                    <>
                      <button
                        ...
                        className="w-full flex items-center justify-center px-4 py-2 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-sm font-medium rounded-xl"
                      >
                        <Edit2 className="w-4 h-4 mr-2" /> Modifier
                      </button>
                      <button
                        onClick={() => setShowDeleteModal(true)}
                        className="w-full flex items-center justify-center px-4 py-2 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 text-sm font-medium rounded-xl"
                      >
          ...
          <div className={`${PANEL} p-5 sm:p-6`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="px-3 py-2 bg-slate-100 dark:bg-slate-700 rounded-xl text-sm font-mono text-slate-700 dark:text-slate-300">
                  {etudiant.matricule}
                </div>
