...
export default function AjouterEtudiantPage() {
  ...
  useEffect(() => {
    filieresApi
      .getAll()
      .then((r) => {
        setFilieres(r.filieres);
        if (r.filieres.length > 0)
          setFormData((p) => ({ ...p, filiere: r.filieres[0].nom }));
      })
      .catch(() => {});
  }, []);
      ...
      {showSuccess && (
        <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 px-4 py-3 rounded-xl flex items-center">
          ...
          <span className="text-sm font-medium">
            Étudiant ajouté avec succès. Redirection...
          </span>
        </div>
      )}
      {error && (
              ...
              <div className="pt-14">
                ...
                <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full flex items-center justify-center px-4 py-2 bg-gradient-to-r from-blue-900 to-blue-700 hover:from-blue-800 hover:to-blue-600 text-white text-sm font-medium rounded-xl disabled:opacity-60"
                  >
                    {submitting && (
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className={PANEL}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { label: "Prénom *", name: "prenom", ph: "Jean", req: true },
