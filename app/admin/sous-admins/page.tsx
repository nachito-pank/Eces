...
export default function SousAdminsPage() {
  ...
  const fetchSousAdmins = async () => {
    ...
    try { const res = await sousAdminsApi.getAll(); setSousAdmins(res.sousAdmins); }
  ...
  const filtered = sousAdmins.filter((a) =>
    [a.nom, a.prenom, a.role].some((v) => (v || '').toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-title font-bold text-slate-900 dark:text-slate-100">Sous-Administrateurs</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Gérez les accès et les droits</p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" placeholder="Rechercher par nom, prénom, rôle..."
          ...
          className="block w-full pl-10 pr-3 py-2 border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-blue-500 text-sm transition-colors shadow-sm" />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.length > 0 ? filtered.map((admin) => (
            <div key={admin.id} onClick={() => router.push(`/admin/sous-admins/${admin.id}`)}
              className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-md hover:-translate-y-1 transition-all duration-200 cursor-pointer group">
              <div className="h-16 bg-gradient-to-r from-blue-900 to-blue-700 relative">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-lg border-4 border-white dark:border-slate-800 shadow-sm absolute -bottom-7 left-5"
              ...
              <div className="pt-9 px-5 pb-5">
                <div className="flex justify-between items-start">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{admin.role}</p>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                  <div className="flex items-center text-xs font-medium text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">
                    <Shield className="w-3 h-3 mr-1" /> Droits d&apos;accès
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {admin.droits?.map((droit: string, idx: number) => (
                      <span key={idx} className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
                        {droit}
                      </span>
                    ))}
                  </div>
            ...
            <div className="col-span-full py-12 text-center bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
              <Search className="w-8 h-8 mb-3 text-slate-300 dark:text-slate-600 mx-auto" />
              <p className="text-sm text-slate-500 dark:text-slate-400">Aucun sous-administrateur trouvé{searchTerm && ` pour "${searchTerm}"`}</p>
            </div>
          )}
        </div>
      )}
